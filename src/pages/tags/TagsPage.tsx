...
import { Tag as TagIcon, Plus, MoreHorizontal, Trash2, Edit2, Check } from 'lucide-react';
    ...
    <div className="flex flex-col gap-3 p-4 rounded-xl border border-neutral-200 dark:border-neutral-800 bg-white dark:bg-neutral-900 shadow-sm relative group hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between gap-4">
        {isEditing ? (
          ...
          <Link to={`/tags/${tag.id}`} className="flex-1 flex items-center gap-2 min-w-0 font-medium text-neutral-900 dark:text-neutral-100 group-hover:text-primary transition-colors">
            <span
      ...
      {showConfirmDelete && (
        <div className="absolute inset-0 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-4 text-center rounded-xl z-10 space-y-3">
          <p className="text-sm font-medium text-neutral-900 dark:text-neutral-100">Delete #{tag.name}?</p>
          <p className="text-xs text-neutral-500 mb-2">It will be removed from all items.</p>
          <div className="flex items-center gap-2">
            <Button size="sm" variant="ghost" onClick={() => setShowConfirmDelete(false)}>Cancel</Button>
            <Button size="sm" variant="destructive" onClick={() => deleteTag(tag.id)} disabled={isDeleting}>Delete</Button>
          </div>
        </div>
      )}
    </div>
    ...
    <PageMotion>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Popover open={openCreate} onOpenChange={setOpenCreate}>
          ...
          <PopoverContent align="end" className="w-64 p-3">
            <p className="text-xs font-semibold uppercase tracking-wider text-neutral-500 mb-3">Create Tag</p>
      ...
      {isLoading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center max-w-sm mx-auto">
          ...
          <h3 className="text-lg font-medium text-neutral-900 dark:text-neutral-100 mb-1">No tags yet</h3>
          <p className="text-sm text-neutral-500 dark:text-neutral-400 mb-6">Create tags to organize your meetings, contacts, tasks, and cards across Crelyzor.</p>
