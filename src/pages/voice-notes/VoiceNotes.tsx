...
export default function VoiceNotes() {
  ...
  // Filter by tab
  if (activeFilter === 'with-transcript') filtered = filtered.filter((vn) => vn.hasTranscript);
  if (activeFilter === 'with-summary') filtered = filtered.filter((vn) => vn.hasSummary);
  if (activeFilter === 'standalone') filtered = filtered.filter((vn) => !vn.meetingId);
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {Object.keys(grouped).length === 0 ? (
        <div className="flex flex-col items-center py-16 text-center">
          ...
          <p className="text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">No voice notes</p>
          <p className="text-xs text-neutral-400 dark:text-neutral-500 max-w-xs">
            Upload a recording to get started. Voice notes will be transcribed and summarized automatically.
          </p>
        </div>
      ) : (
              ...
              <div key={date}>
                ...
                <div className="space-y-2">
                  {notes.map((vn) => {
                    ...
                    return (
                      <Card key={vn.id} className="border-neutral-200 dark:border-neutral-800 hover:border-neutral-300 dark:hover:border-neutral-700 transition-colors">
                        <CardContent className="p-4">
                          <div className="flex items-start gap-3">
                            {/* Play button */}
                            <button
                              onClick={() => setPlayingId(isPlaying ? null : vn.id)}
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {/* SMA indicators */}
                                {vn.hasTranscript && (
                                  <div className="w-5 h-5 rounded-full bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center" title="Transcribed">
                              ...
                              <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                                ...
                                <span className="text-[11px] text-neutral-400 dark:text-neutral-500">{vn.fileSize}</span>
                                {vn.meetingTitle && (
                                  <button
                                    onClick={() => vn.meetingId && navigate(`/meetings/${vn.meetingId}`)}
                                ...
                                {isPersonalView && vn.orgSource && !vn.orgSource.isPersonal && (
                              ...
                              {isPlaying && (
                                <div className="mt-3 flex items-center gap-2">
                                  ...
                                  <span className="text-[10px] text-neutral-400 font-mono">14:32 / {vn.duration}</span>
                                </div>
                              )}
                            </div>
