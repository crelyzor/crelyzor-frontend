...
export default function SignIn() {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden bg-background text-foreground">
      ...
      {/* Navbar — mirrors public HomeNavbar exactly */}
      <motion.nav
        initial={{ opacity: 0, y: -8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4, ease: [0.25, 0.1, 0.25, 1] }}
        className="fixed top-0 left-0 right-0 z-50 px-4 py-4 sm:px-8 sm:py-5 flex items-center justify-between bg-background/80 backdrop-blur-xl"
        style={{
          borderBottom:
            '1px solid color-mix(in srgb, var(--color-border) 50%, transparent)',
        }}
      >
        <a href={PUBLIC_URL} className="flex items-center">
          <img
      ...
      <div className="flex-1 flex flex-col pt-[64px] px-8 sm:px-16 md:px-24">
