...
const PRIORITY_OPTIONS: { value: Priority; label: string; color: string; bg: string }[] = [
  { value: 'HIGH',   label: 'High',   color: 'text-red-400',   bg: 'bg-red-500/10 border-red-500/30 text-red-400' },
  { value: 'MEDIUM', label: 'Medium', color: 'text-amber-400', bg: 'bg-amber-500/10 border-amber-500/30 text-amber-400' },
  { value: 'LOW',    label: 'Low',    color: 'text-blue-400',  bg: 'bg-blue-500/10 border-blue-500/30 text-blue-400' },
];
...
export function CreateTaskModal({ open, onClose, navigateOnSuccess }: Props) {
  ...
  }, [title, description, dueDate, priority, createTask, onClose, navigateOnSuccess, navigate]);
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            onClick={() => { if (activePopover) { setActivePopover(null); return; } onClose(); }}
              ...
              <div className="px-3 pb-3 flex items-center gap-2">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {dueDate && (
                      <span onClick={handleClearDate} className="ml-0.5 opacity-60 hover:opacity-100 transition-opacity">
                        <X className="w-3 h-3" />
                      </span>
                    )}
                  </button>
                    ...
                    {activePopover === 'date' && (
                      ...
                      >
                        {DATE_PRESETS.map((preset) => (
                          <button
                            key={preset.label}
                            onClick={() => handleSetDate(preset.label, toISODate(preset.days))}
                ...
                <div className="relative">
                  ...
                  >
                    <Flag className={`w-3.5 h-3.5 shrink-0 ${priorityInfo?.color ?? ''}`} />
                    <span>{priorityInfo?.label ?? 'Priority'}</span>
                    {priority && (
                      <span onClick={handleClearPriority} className="ml-0.5 opacity-60 hover:opacity-100 transition-opacity">
                        <X className="w-3 h-3" />
                      </span>
                    )}
                  </button>
                  ...
                  <AnimatePresence>
                    {activePopover === 'priority' && (
                      ...
                      >
                        {PRIORITY_OPTIONS.map((opt) => (
                          <button
                            ...
                            className={`w-full flex items-center gap-2 px-3 py-2 text-[12px] hover:bg-white/5 transition-colors ${
                              priority === opt.value ? 'text-white' : 'text-neutral-300'
                            }`}
                          >
                        ...
                        <div className="h-px bg-white/5 mx-2 my-1" />
                        <button
                          onClick={() => { setPriority(null); setManualPriority(false); setActivePopover(null); }}
